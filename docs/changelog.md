...
## 2026-05-06 — Phase 3 进行中

### Phase 3: 打轴核心（进行中）

- 实现 TimelineCard 时间轴卡片（虚拟滚动）
- 实现 101行×4列动态表格
- 实现虚拟滚动：只渲染可视区域（200行），支持任意长度视频
- 实现字幕条渲染：根据持续时间显示不同颜色
- 实现快捷键支持：Q/W/E/R 调整轴端，5切割，Delete删除
- 实现右键上下文菜单：合并、切割、删除、复制、剪切、粘贴
- 实现撤销/重做（Ctrl+Z/Y）
- 实现叠轴检测：安全/警告/阻止
- 实现间隔设置：下拉选择 10ms~1s
- 实现打轴工具栏：间隔设置、新建/合并/切割/撤销/重做按钮
- 实现点击跳转并暂停播放
- 实现 Shift+左键拖动创建轴
- 实现 Shift+右键拖动取消轴
- 实现轴管理：自动命名、颜色区分、右键菜单重命名/删除
- 实现滚动边界处理：边界时移动视频位置

### 已知问题

详见 [docs/todo.md](todo.md) — 存在选轴拖动卡顿、滚动时轴颜色偏移等问题
