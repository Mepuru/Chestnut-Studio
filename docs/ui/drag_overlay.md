...
│                                             │
│  ┌───────────────────────────────────────┐  │
│  │     📝  字幕文件（.srt / .ass）       │  │  ← 拖入字幕时高亮
│  └───────────────────────────────────────┘  │
│                                             │
└─────────────────────────────────────────────┘
```

拖入视频文件时，视频区域高亮（蓝色边框），字幕区域变暗。
拖入字幕文件时，字幕区域高亮（绿色边框），视频区域变暗。
...
| 方法 | 说明 |
|------|------|
| `update_for_files(paths)` | 根据文件类型高亮对应区域 |
| `handle_drop(paths)` | 处理放下事件，分发信号 |
| `_detect_file_type(paths)` | 静态方法，判断文件类型 |
| `_create_zone(border, bg, color)` | 静态方法，创建拖放区域 |
| `_set_zone_active(zone, active, ...)` | 设置区域高亮/变暗状态 |
