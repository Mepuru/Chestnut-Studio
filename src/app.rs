use iced::widget::{
    ...
    pick_list, responsive, row, text, PaneGrid,
};
...
impl ChestnutStudio {
    ...
    fn view_menu_bar(&self) -> Element<'_, Message> {
        let layout_picker = pick_list(
            ...
            Some(self.state.current_preset()),
            Message::ApplyLayout,
        )
        .placeholder("布局")
        .font(FONT)
        ...
        let pane_grid =
            PaneGrid::new(&self.state.panes, |id, pane, is_maximized| {
                ...
                let close_btn = button(
                    text("x").font(FONT).size(11).color(TEXT_SECONDARY),
                )
                ...
                let maximize_btn = if is_maximized {
                    ...
                    button(
                        text("最大化").font(FONT).size(11).color(TEXT_SECONDARY),
                    )
