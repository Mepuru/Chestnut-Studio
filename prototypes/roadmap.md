...
## Phase 3 — 打轴功能（预计 3~4 天）

**目标：** 实现打轴功能（在音频波形区打轴，时间轴列表显示）
...
| 3.7 | 实现撤销/重做 | Ctrl+Z/Y | 操作可撤销重做 |
| 3.8 | 实现波形字幕覆盖 | 波形上显示已打轴的字幕条 | 字幕条在波形上可见 |

**里程碑：** 打开视频 → 在波形区打轴 → 时间轴列表显示 → 可查看/编辑/锁定
...
| 4.10 | 实现字幕实时预览 | 视频上显示字幕 | 编辑后视频上字幕实时更新 |

**里程碑：** 完整工作流：打开视频 → 打轴 → 填写翻译 → 导出 ASS 文件
...
Week 3:  Phase 3             →  字幕列表可用
Week 4:  Phase 4             →  完整工作流
Week 5:  Phase 5             →  可打包发布
```
